...
export default function StatsCard({ title, value, subtitle, icon }: Props) {
  return (
    <Card className="group flex aspect-square min-h-36 flex-col rounded-2xl shadow-sm transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-md active:scale-[1.01] sm:min-h-40">
      <CardHeader className="flex flex-row items-start justify-between gap-3 p-3.5 sm:p-4">
        <CardDescription className="text-xs font-medium tracking-normal sm:text-sm">
          {title}
        </CardDescription>
